...
trait Friendable{
...

    {
            ...
            if($this->is_friends_with($user_requested_id)){
                ...
                return "already friends with ". \App\User::find($user_requested_id)->name;
            }

            if($this->has_pending_friend_request_sent_to($user_requested_id) == 1 ){

                return 'Already sent request to this user';
            }
    ...
    public function friends(){
        ...
        $f = Friendship::where('status', 1)
                            ->where('requester', $this->id)
                            ->get();

        foreach($f as $friendship):

           array_push($friendList1, \App\User::find($friendship->user_requested));
        ...
        foreach($f2 as $friendship):
            array_push($friendList2, \App\User::find($friendship->requester));
        endforeach;

        return array_merge($friendList1, $friendList2);
    }
